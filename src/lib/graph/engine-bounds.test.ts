...
describe("boundary geometry during a drag", () => {
  it("leaves the excluded node out of a boundary body's rect, and puts it back on release", () => {
    ...
    // contains.
    //
    // Releasing means BOTH halves of what handlePointerUp does — unpin and
    // clear the exclusion. A tab is kept out of its boxes’ geometry by two
    // independent guards the drag handler sets and clears together: this
    // explicit exclusion, and `isPointerHeld` (engine.ts), which reads d3’s
    // fx/fy directly and also stops the boundary layer translating a node
    // the pointer is holding. Dropping only one of them here would leave
    // "b" pinned at 5000 and still, correctly, outside the rect.
    sim.unpin("b");
