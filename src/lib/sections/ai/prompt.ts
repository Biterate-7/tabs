...
export function buildOrganizePrompt(sections: Section[], tabs: OrganizePromptTab[]): string {
  ...
  return [
    ...
    "",
    "MEMBERSHIP — judge every tab on its own evidence:",
    "- The question is \"is THIS tab actually part of that subject?\", never \"could this tab plausibly belong to something already in the tree?\". A section is not a bucket to fill out.",
    "- Do not put a tab under a section named after a specific site, product or project unless the tab's OWN domain, URL or title ties it there. Sharing a broad theme is not evidence: a song-lyrics page and a school-management platform are both loosely \"school-adjacent\" and still belong nowhere near each other, and two different AI tools are not the same project.",
    "- Arriving in the same batch as related tabs, or having been open beside them, is never evidence.",
    "- When you can't support the specific placement, stop at the broad category with confidence \"low\". Leaving a tab unplaced is a better outcome than placing it somewhere it doesn't belong.",
    "",
    "CONFIDENCE",
...
export function buildClusterOrganizePrompt(sections: Section[], clusters: OrganizeClusterInput[]): string {
  ...
  return [
    ...
    "",
    "MEMBERSHIP — the path has to fit the WHOLE cluster:",
    "- Judge each cluster on its own sample titles and domains. Do not file it under an existing section named after a specific site, product or project unless those titles/domains actually tie it there; sharing a broad theme with that section is not evidence.",
    "- If a specific section would fit only part of the cluster, choose the broader path that fits all of it rather than the narrow one that fits some.",
    "",
    "NAMING",
    "- Names must be short, clean, human-readable nouns or noun phrases (e.g. \"Physics\", \"TabDump\", \"S2 Orbit Research\"). Never vague or padded names like \"Miscellaneous Physics\", \"Physics Resources & Materials\", \"Random Research\", \"Web Things\", or \"Other Research\".",
    "- Never use the name \"Other\" for any path segment — it is reserved and will be ignored.",
