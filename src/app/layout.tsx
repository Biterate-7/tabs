 ...
 * render two families. That was invisible while `/` was the only route and the
 * app sat behind a hydration gate; it stops being invisible the moment a
 * statically-served public landing page pays it on first paint.
 *
 * `preload: false` keeps the @font-face rule and the CSS variable, so a font a
 * user picks in Settings still resolves and still self-hosts — the browser just
 * fetches it when a rule actually applies it. The two below stay eager because
 * they are the defaults (see --tabdump-font-ui/-mono in globals.css) and the
 * landing page pins them.
 */
...
export const metadata: Metadata = {
  // Absolute URLs for every page's canonical/Open Graph entries resolve against
