...
export const DEMO_TABS: DemoTab[] = [
  ...
  // Context for the agent run further down the page. A coding agent's context
  // is whatever the person had open while the work happened, so that run links
  // to real rows of this corpus rather than to tabs invented for it.
  tab("developer.mozilla.org", "Set-Cookie — HTTP", "projects", "Development", "Documentation"),
  tab("github.com", "googleapis/google-auth-library-nodejs", "projects", "Development", "GitHub"),
  tab("web.dev", "SameSite cookies explained", "projects", "Development", "Documentation"),
...
/**
 * Looks a tab up by its title.
 *
 * The agent demos point at *particular* rows of the corpus — "this run used
 * these three tabs as context" — and the ids above are assigned by position,
 * so hard-coding `d24` would silently point somewhere else the first time a
 * tab is inserted above it. Throwing rather than returning undefined is
 * deliberate: a demo wired to a tab that no longer exists is a bug to find at
 * build time, not a gap to render around.
 */
export function demoTabByTitle(title: string): DemoTab {
  const found = DEMO_UNIQUE_TABS.find((t) => t.title === title)
  if (!found) throw new Error(`demoTabByTitle: no tab titled ${JSON.stringify(title)}`)
  return found
}
