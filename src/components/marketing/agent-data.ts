...
export const DEMO_AGENT_STATE: AgentState = {
  ...
  // Empty, and truthfully so. Task-level evidence is recorded when a provider
  // reports which work item a file operation or event belonged to; nothing on
  // this page renders that attribution, so inventing rows here would put a
  // claim in the fixture that no section is prepared to show. An empty array
  // is a valid state the domain documents — "nothing reported which task a
  // thing belonged to" — not a gap waiting to be filled.
  workItemEvidence: [],
}
