#!/usr/bin/env node
/**
 * Applies the remote agent runtime schema (src/lib/agents/remote/schema.sql).
 *
 ...
 * remote schema shares that database and that connection pool deliberately.
...
const SCHEMA_PATH = path.join(ROOT, "src", "lib", "agents", "remote", "schema.sql");
...
async function main() {
  ...
  try {
    ...
    // never is.
    console.log(`Remote agent runtime schema applied (via ${connection.name}).`);
  } catch (error) {
